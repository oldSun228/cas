...
public final class LogoutManagerImpl implements LogoutManager {
    ...
    public LogoutManagerImpl(final ServicesManager servicesManager, final HttpClient httpClient,
                             final LogoutMessageCreator logoutMessageBuilder, final DataSource dataSource) {
    ...
    public List<LogoutRequest> performLogout(final TicketGrantingTicket ticket) {
        ...
        Map<String, Object> attrMap = ticket.getAuthentication().getPrincipal().getAttributes();
        
        // 清除令牌
        String token = (String) attrMap.get("token");
//        getJdbcTemplate().execute(sql);
        if (getJdbcTemplate() != null) {
        	try {
        		getJdbcTemplate().update(sqlDeleteToken, token);
